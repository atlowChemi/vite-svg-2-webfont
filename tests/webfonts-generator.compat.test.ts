// oxlint-disable jest/require-to-throw-message
...
async function runSideBySideWithDifferentDest(options: GenerateWebfontsInputOptions) {
    ...
    if (upstreamDest) cleanupDirs.add(upstreamDest);
    if (newCoreDest) cleanupDirs.add(newCoreDest);
...
describe('compat:webfonts-generator:side-by-side', () => {
    ...
    describe.each(['html', 'css'] as const)('%s template parity', templateType => {
