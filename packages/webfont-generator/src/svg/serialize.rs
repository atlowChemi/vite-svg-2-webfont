...
pub(crate) fn optimize_path_data(path_data: &str) -> String {
    ...
    let options = Options::all().difference(Options::CloseSegments | Options::UniteSegments);
    path = path.optimize(options, FillRule::default(), &Tolerance::default());
    path.to_string()
}
